...
async function upsertEffectifDeca(
  ...
  if (!effectifFound) {
    try {
      ...
      await createMissionLocaleSnapshot({ ...effectif, _id: id }, "TEST A CHANGER");
      count.created++;
    } catch (err) {
    ...
    await createMissionLocaleSnapshot({ ...effectif, _id: effectifFound._id, transmitted_at }, "TEST A CHANGER");
    count.updated++;
  }
}
