...
const adminTransmissionByDayColumnDefs: AccessorKeyColumnDef<any, any>[] = [
  ...
  },
  {
    ...
    header: () => "Voir",
    cell: ({ row }) => (
      <Link href={`/admin/transmissions/${row.original.day as any}`}>
...
const AdminTransmissionByDayTable = () => {
  ...
  const { data, error, isFetching } = useQuery({
    queryKey: ["transmissions-admin", pagination],
    queryFn: () =>
      _get(`/api/v1/admin/transmissions`, {
        params: {
...
export default AdminTransmissionByDayTable;
