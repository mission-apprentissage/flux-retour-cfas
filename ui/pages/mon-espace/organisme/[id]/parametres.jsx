...
const PageParametresOrganisme = () => {
  const router = useRouter();
  const { organisme, isloaded } = useOrganisme(router.query.id);
  const title = "Paramètres de son organisme";
          ...
          <Box mt={4}>
            {organisme ? (
              hasContextAccessTo(organisme, "organisme/page_parametres") ? (
                <ParametresOrganisme organisme={organisme} title="Paramètres de son organisme" />
              ) : (
...
export default withAuth(PageParametresOrganisme);
