...
export const hydrateOrganismesFormations = async () => {
  // Récupère tous les organismes distinct dans les dossiersApprenants
  ...
  // Parse tous les organismes id distinct dans les dossiersApprenants
  await asyncForEach(allOrganismesId, async (organisme_id) => {
    // Récupère toutes les formations distinctes pour cet organisme dans les dossiersApprenants
    ...
    if (formationsIdForOrganisme.length > 0) {
      ...
      await asyncForEach(formationsIdForOrganisme, async (formation_id) => {
        ...
        // Ajout à la liste des formation d'un item contenant formation_id et liste des organismes liés
      ...
      await updateOrganisme(organisme_id, { formations: formationsArray });
    }
  });
};
