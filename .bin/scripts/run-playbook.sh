...
function runPlaybook() {
  ...
  if [[ -z "${ANSIBLE_BECOME_PASS:-}" ]]; then
    if [[ $* != *"pass"* ]]; then
        local become_pass=$(op --account mission-apprentissage account read op://Private/${PRODUCT_NAME}-$ENV_FILTER/password 2> /dev/null);
  ...
  if [[ -z "${ANSIBLE_REMOTE_USER:-}" ]]; then
    if [[ $* != *"--user"* ]]; then
        local username=$(op --account mission-apprentissage account read op://Private/${PRODUCT_NAME}-$ENV_FILTER/username 2> /dev/null);
        if [ -z $username ]; then
