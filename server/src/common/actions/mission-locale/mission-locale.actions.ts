...
export const createMissionLocaleSnapshot = async (
  ...
  if (mongoInfo.lastErrorObject?.n > 0) {
    ...
    if (upsertedId) {
      ...
      if (softDeletedId) {
        await mergeAndSoftDeleteDuplicates(new ObjectId(upsertedId), [softDeletedId]);
      }
  ...
  const effectifsMissionLocaleAggregation = [
    ...(await missionLocaleBaseAggregation(organisation)),
    {
      $group: {
        ...
        nouveau_projet: withCondition({
          $or: [
            { $eq: ["$situation", SITUATION_ENUM.NOUVEAU_PROJET] },
            { $eq: ["$situation", SITUATION_ENUM.NOUVEAU_CONTRAT] },
          ],
        }),
