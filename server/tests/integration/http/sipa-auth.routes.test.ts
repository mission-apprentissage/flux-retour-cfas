...
const statusOf = (err: unknown) => (err as Boom).output.statusCode;

function runSipaMiddleware(authHeader?: string): Promise<{ err: unknown; req: Request }> {
  return new Promise((resolve) => {
    const req = { headers: authHeader ? { authorization: authHeader } : {} } as unknown as Request;
    requireSipaAuthentication()(req, {} as Response, (err?: unknown) => resolve({ err, req }));
  });
}
...
describe("Authentification SIPA", () => {
  ...
  describe("POST /api/v2/auth/login", () => {
    it("Retourne un token JWT et expiresIn sur identifiants valides, et met à jour last_connection", async () => {
      ...
      const decoded = jwt.verify(response.data.token, SIPA_JWT_SECRET, { complete: true });
      const payload = decoded.payload as JwtPayload;
      assert.strictEqual(decoded.header.alg, "HS256");
      assert.strictEqual(payload.scope, "sipa");
      assert.strictEqual(payload.sub, SIPA_USERNAME);
      assert.strictEqual(payload.iss, config.appName);
      assert.strictEqual(Number(payload.exp) - Number(payload.iat), 604800);
  ...
  describe("Middleware requireSipaAuthentication", () => {
    ...
    it("401 sans header Authorization", async () => {
      ...
      assert.strictEqual(statusOf(err), 401);
    });
    ...
    it("403 si scope absent", async () => {
      ...
      const { err } = await runSipaMiddleware(`Bearer ${token}`);
      assert.strictEqual(statusOf(err), 403);
    });
  ...
  describe("createSipaUser / deleteSipaUser (actions CLI)", () => {
    ...
    it("Refuse un username en doublon", async () => {
      await assert.rejects(
        ...
        (err: Boom) => err.output.statusCode === 409
      );
    });

    it("Refuse un mot de passe trop court (RGS : 20 caractères minimum)", async () => {
      await assert.rejects(createSipaUser("autre-compte", "trop-court"), (err: Boom) => err.output.statusCode === 400);
    });
    ...
    it("Refuse un mot de passe trop long (>128, borne du login : un compte créé doit pouvoir se connecter)", async () => {
      await assert.rejects(
        ...
        (err: Boom) => err.output.statusCode === 400
      );
    });
    ...
    it("Supprime un compte existant, erreur si introuvable", async () => {
      ...
      await assert.rejects(deleteSipaUser(SIPA_USERNAME), (err: Boom) => err.output.statusCode === 404);
    });
  });
