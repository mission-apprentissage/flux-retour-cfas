...
const getExportAnonymizedEventNameFromFilters = (filters) => {
  if ("etablissement_num_region" in filters) {
    return USER_EVENTS_ACTIONS.EXPORT.ANONYMIZED_DATA_LISTS.TERRITOIRE_REGION;
  }
  if ("etablissement_num_departement" in filters) {
    return USER_EVENTS_ACTIONS.EXPORT.ANONYMIZED_DATA_LISTS.TERRITOIRE_DEPARTEMENT;
  }
  if ("etablissement_reseaux" in filters) {
    return USER_EVENTS_ACTIONS.EXPORT.ANONYMIZED_DATA_LISTS.RESEAU;
  }
  if ("formation_cfd" in filters) {
    return USER_EVENTS_ACTIONS.EXPORT.ANONYMIZED_DATA_LISTS.FORMATION;
  }
  if ("uai_etablissement" in filters) {
    return USER_EVENTS_ACTIONS.EXPORT.ANONYMIZED_DATA_LISTS.CFA;
  }
  return USER_EVENTS_ACTIONS.EXPORT.ANONYMIZED_DATA_LISTS.TERRITOIRE_NATIONAL;
};
