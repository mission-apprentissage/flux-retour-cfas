...
function useOrganismeAcces() {
  ...
  const [
    { data: organismeContributors, isLoading: isLoadingContributors, isFetching: isFetchingContributors },
...
const InviteModal = ({ title, size = "md", isOpen, onClose, onInvite, defaultRoleValue = undefined }) => {
  ...
  const organisme = useRecoilValue(organismeAtom);
  const { organismeContributors, roles, isLoading, initRole } = useOrganismeAcces();

  const onAddContributor = useMutation(
    ({ userEmail, roleId, acl = [] }) => {
      return _post(`/api/v1/organisme/contributors`, {
    ...
    {
      onSuccess: () => queryClient.invalidateQueries("organismeContributors"),
    }
  );

  const onChangeContributorRole = useMutation(
    ({ userEmail, roleId, acl = [] }) => {
      return _put(`/api/v1/organisme/contributors`, {
        organisme_id: organisme._id,
        userEmail,
        roleId,
        acl,
  ...
  const onDeleteContributor = useMutation(
    (contributor) => {
      ...
      if (remove) {
        try {
          return _delete(
            `/api/v1/organisme/contributors?organisme_id=${organisme._id}&userEmail=${contributor.user.email.replace(
              "+",
              "%2B"
        ...
        <ModalBody p={0}>
          ...
          {!isLoading && (
            <Box px={[4, 8]} mb={5}>
              ...
              <Flex mt={8}>
                <Table
                  data={organismeContributors.map((d) => ({
                    Username: {
                      Header: "Utilisateur",
                      width: 120,
                      value: "",
                  ...
                  components={{
                    Username: (value, i) => {
                      const constrib = organismeContributors[i];
                    ...
                    Role: (value, i) => {
                      return (
                        <Box w="full">
                          {organismeContributors[i].owner && <Text as="i">Propriétaire</Text>}
                          {!organismeContributors[i].owner && (
                            <Select
                              size="sm"
                              onClick={(e) => {
                                e.stopPropagation();
                              ...
                              onChange={async (e) => {
                                ...
                                if (roleId === "custom") {
                                  ...
                                  onChangeContributorRole.mutate({
                                    userEmail: organismeContributors[i].user.email,
                                    roleId,
                                  });
                                }
                              }}
                              ...
                              value={
                                organismeContributors[i].permission.name === "wks.custom"
                                  ? "custom"
                                  : organismeContributors[i].permission._id
                              }
                            >
                              {roles.map((role, j) => {
                                return (
                        ...
                        <Center pl={5}>
                          {!organismeContributors[i].owner && (
                            <CloseIcon
                              color="bluefrance"
                              onClick={async () => {
                                onDeleteContributor.mutate(organismeContributors[i]);
                              }}
                              cursor="pointer"
                            />
                          )}
